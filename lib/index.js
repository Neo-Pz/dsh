...
import { chmodSync, copyFileSync, mkdirSync as mkdirSync2, statSync, writeFileSync as writeFileSync2 } from "node:fs";
...
function nodeHome(join2, workspace) {
  return workspace;
}
function principalHome(join2, workspace) {
...
function declarationsPath(join2, workspace) {
  return join2(workspace, ".iflow", DECLARATIONS);
}
async function loadDeclarations(ctx, join2, workspace) {
  try {
    const resolved = await ctx.fs.resolve(declarationsPath(join2, workspace));
...
function homeForSigning(join2, workspace, declarations, context, nodeDid) {
  ...
  if (context.did) {
    ...
    if (declarations.principal && declarations.principal.did === context.did) {
      return principalHome(join2, workspace);
    }
...
function staleBinaryAdvice(binPath, cachePath, missing) {
  return `iFlow: the identity binary at ${binPath} is older than this plugin \u2014 it cannot ${missing.map((command) => IFI_CAPABILITIES[command]).join(" or ")}. Everything else works. To fix it, delete ${cachePath} and run iflow_fetch_identity, or point IFLOW_ID_PATH at a current build.`;
}
...
function createRelayTransport(io) {
  ...
  return { seal, open, send: send2, inbox, ack, heartbeat, directory, drain };
}
function startRelayPolling({ transport, settings, agents, deliver, intervalMs = 15e3, logger = console }) {
  let running = false;
  const tick = async () => {
    if (running) return;
    running = true;
...
var index_default = {
  ...
  apply(ctx, config = {}) {
    ...
    function participantsFor(conversation, initiator) {
      const mine = { agentId: selfAgentId(), did: state.identityDid ?? void 0, role: "recipient", joinedAt: iso() };
      const theirs = {
      ...
      const request = {
        ...
        params: {
          ...
          metadata: {
            ...
            actorType: "human",
            origin: "keyboard",
            principalId: state.identityDid ?? void 0,
    ...
    async function fetchWithNode(dest, url) {
      ...
      writeFileSync2(dest, Buffer.from(await response.arrayBuffer()));
    }
      ...
      defineTool({
        ...
        output: {
          ...
          render: (_args, value) => [
            {
              type: "text",
              text: value.ok ? `iFlow identity ready: ${value.did ?? "no identity created yet"} (${value.path})` : `iFlow identity unavailable: ${value.error}`
            }
          ]
        },
        async execute() {
