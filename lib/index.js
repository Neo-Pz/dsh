...
var EMITTABLE_ISSUER_KINDS = /* @__PURE__ */ new Set(["agent"]);
var STORED_ISSUER_KINDS = /* @__PURE__ */ new Set(["agent", "human", "system"]);
...
function validateEvent(candidate, options = {}) {
...
function reduceKnown(state, event) {
  ...
  if (isEventOfType(event, "delivery.accepted") || isEventOfType(event, "delivery.rejected")) {
    ...
    const ruledByAgentId = isEventOfType(event, "delivery.accepted") ? event.payload.acceptedByAgentId : event.payload.rejectedByAgentId;
...
function contentOriginOf(event) {
  if (isEventOfType(event, "conversation.message_sent") || isEventOfType(event, "conversation.message_received")) {
...
function projectActivityFeed(state, options) {
  const entries = state.recent.map((event) => ({
    ...
    legacyActorKind: event.issuer.kind === "agent" ? void 0 : event.issuer.kind,
...
function inspectEvents(events) {
  ...
  for (const raw of events) {
    ...
    const message = (type === "user/message" ? data : data?.message) ?? void 0;
    ...
    findings.push({
      seq,
      type,
      reason: ours ? "written by an older iFlow: an assistant source with no kind" : "source has no kind, and was not written by iFlow",
      repairable: ours
    });
  }
  return findings;
...
// src/conversation/permissions.ts
var PERMISSIONS = "permissions.json";
function permissionsPath(join2, workspace) {
  return join2(workspace, ".iflow", PERMISSIONS);
}
var pairKey = (localAgentDid, peerAgentDid) => `${localAgentDid}|${peerAgentDid}`;
function emptyPermissions() {
  return { pairs: {} };
}
async function loadPermissions(ctx, join2, workspace) {
  try {
    const resolved = await ctx.fs.resolve(permissionsPath(join2, workspace));
...
async function savePermissions(ctx, join2, workspace, permissions) {
  const resolved = await ctx.fs.resolve(permissionsPath(join2, workspace));
  await ctx.fs.writeText(resolved, `${JSON.stringify(permissions, null, 2)}
`);
}
function pairMessagingState(permissions, localAgentDid, peerAgentDid) {
...
function allowPair(permissions, { localAgentDid, peerAgentDid, localAgentId, peerLabel, now }) {
  if (!localAgentDid || !peerAgentDid) return null;
  ...
  const pair = {
    ...
    messaging: "allowed",
    grantedAt: now ?? (/* @__PURE__ */ new Date()).toISOString(),
    grantedBy: "human",
    revokedAt: null
  };
...
function revokePair(permissions, localAgentDid, peerAgentDid, now) {
  const pair = permissions.pairs[pairKey(localAgentDid, peerAgentDid)];
  if (!pair || pair.revokedAt) return null;
  pair.revokedAt = now ?? (/* @__PURE__ */ new Date()).toISOString();
  pair.messaging = "blocked";
  return pair;
}
function forgetPair(permissions, localAgentDid, peerAgentDid) {
  if (!localAgentDid || !peerAgentDid) return null;
  const key = pairKey(localAgentDid, peerAgentDid);
...
function allowedPairs(permissions) {
  return Object.values(permissions.pairs).filter((pair) => !pair.revokedAt && pair.messaging === "allowed").sort((a, b) => String(b.grantedAt).localeCompare(String(a.grantedAt)));
}
...
function trustDecision(trust, { peerLabel, signerDid, conversation, pairMessaging } = {}) {
...
function normalize(id, value) {
  return {
    ...
    communicationState: value.communicationState === "reauthorization_required" ? "reauthorization_required" : "active",
...
function collapseToCounterparties(conversations, localAgentId) {
  ...
  for (const candidate of Object.values(conversations)) {
    ...
    const better = !held || liveNow && !liveHeld || liveNow === liveHeld && String(candidate.updatedAt) > String(held.updatedAt);
...
var index_default = {
  ...
  apply(ctx, config = {}) {
    ...
    const conversationsReady = Promise.all([
      ...
      loadPermissions(ctx, join, workspace).then((permissions) => {
...
${text}`);
    ...
    async function handleSendMessage(params, signerDid, grant, arrival) {
      ...
      if (decision === "ask") {
        conversation.state = "pending";
        conversation.communicationState = pairState === "revoked" ? "reauthorization_required" : "active";
      ...
      if (conversation.state === "pending") {
        observeEdge(
          "conversation.accepted",
          (observer) => observer.conversationAccepted({ conversationId, acceptedByAgentId: selfAgentId(), decidedBy: "policy" })
        );
      }
      if (firstSighting) {
        observeEdge(
      ...
      state.outgoing.set(taskId, { controller, done: void 0, conversationId });
    ...
    async function acceptConversation(conversationId, { decidedBy = "human" } = {}) {
      ...
      observeEdge(
        "conversation.accepted",
        (observer) => observer.conversationAccepted({ conversationId, acceptedByAgentId: selfAgentId(), decidedBy })
      );
      observeEdge(
        "relation.recorded",
        (observer) => observer.relationRecorded({
      ...
      await persistConversations();
      const controller = makeAbortController();
      state.outgoing.set(parked.taskId, { controller, done: void 0, conversationId });
    ...
    async function resetPair(localAgentDid, peerAgentDid, confirmation) {
      ...
      const conversationIds = Object.values(state.conversations).filter((conversation) => belongsToPair(conversation, localAgentDid, peerAgentDid)).map((conversation) => conversation.conversationId);
      ...
      const web = webIntentQueue ? await webIntentQueue.forgetConversations(conversationIds) : { intents: 0, bindings: 0 };
      return {
        ok: true,
        localAgentDid,
        peerAgentDid,
...
${warnings.map((w) => `  ! ${w}`).join("\n")}`;
  ...
  identity pinned: ${didFingerprint(value.did)}` : "") : `iFlow: ${value.error}`
      ...
      defineTool({
        ...
        description: "iFlow: see conversations with other agents and answer the ones waiting on you. A first message from an unknown agent is held \u2014 no session, no model, no tools \u2014 until you accept it here. Actions: list (default), accept, reject, trust (auto-accept a peer from now on), block, reset_pair (local diagnostic only).",
        parameters: {
...
${value.text}` : `iFlow call failed: ${value.error}`
        ...
        async execute(args) {
          ...
          if (TERMINAL_TASK_STATES.has(task.status.state)) {
            ...
            await mirrorExchange(outbound, [
              ...
              { side: "peer", messageId: `${messageId}:reply`, text: text2, author: "agent" }
            ]);
            return {
      ...
      defineTool({
        ...
        description: "iFlow: find local DSH sessions this plugin made unloadable by writing an assistant message with no source.kind, and optionally repair them. Reports only unless apply is true. Every repair writes a .backup beside the file first.",
        parameters: {
        ...
        output: {
          ...
          render: (_args, value) => [
            {
              type: "text",
              text: value.ok ? `${value.summary}${value.applied ? "" : " (report only; pass apply: true to fix, a .backup is written first)"}` + (value.sessions ?? []).map((entry) => {
                const lines = (entry.findings ?? []).join("\n  ");
                return `

${entry.sessionId} (${entry.workspace})
  ${lines}`;
              }).join("") : `could not scan: ${value.error}`
            }
          ]
        },
        async handler(args) {
          const { readdirSync, existsSync: existsSync2, statSync: statSync2, readFileSync: readFileSync4, writeFileSync: writeFileSync4, copyFileSync: copyFileSync2 } = await import("node:fs");
          ...
          if (!existsSync2(root)) return { ok: false, error: `no session store at ${root}` };
          const decodeAll = (bytes) => {
            ...
            while (rest.length > 0) {
              let decoded;
              try {
                decoded = zstdDecompressSync(rest);
              } catch {
                break;
              }
          ...
          const findNextFrame = (bytes, from) => {
            for (let i = from; i + 4 <= bytes.length; i++) {
              if (bytes[i] === 40 && bytes[i + 1] === 181 && bytes[i + 2] === 47 && bytes[i + 3] === 253) return i;
            }
            return -1;
          };
          const scanned = [];
          for (const ws of readdirSync(root)) {
            ...
            if (!statSync2(wsPath).isDirectory()) continue;
            for (const id of readdirSync(wsPath)) {
              ...
              let events;
              try {
                events = decodeAll(readFileSync4(file)).split("\n").filter((line) => line.trim()).map((line) => JSON.parse(line));
              } catch (err) {
              ...
              if (args.apply === true && entry.repairable > 0) {
                ...
                copyFileSync2(file, `${file}.backup`);
                const rewritten = `${fixed.map((event) => JSON.stringify(event)).join("\n")}
`;
                writeFileSync4(file, zstdCompressSync(Buffer.from(rewritten, "utf8")));
          ...
          return {
            ...
            sessions: scanned,
            summary: scanned.length === 0 ? "no session has an unreadable message source" : `${scanned.length} session(s) affected; ${scanned.reduce((n, s) => n + (s.repaired?.length ?? 0), 0)} event(s) repaired`
          };
        }
      }),
      defineTool({
  ...
  expires: ${value.expiresAt}` : value.revokeStatus ? `grant ${value.grantId}: ${value.revokeStatus}` : `grant verified \u2713 (grant_id ${value.grantId}, issuer ${value.issuer}, delegate ${value.delegate}, level ${value.level})`
    ...
    function privateMessages(conversation, events, cursor, limit) {
      const all = events.flatMap((event, index) => {
        ...
        return [{
          ...
          side,
          authorAgentId: marked?.authorAgentId ?? (side === "self" ? conversation.localAgentId : conversation.peerAgentId),
          ...
          representedBy: marked?.represents ?? (side === "self" ? conversation.localAgentId : conversation.peerAgentId),
    ...
    async function executeWebConversationIntent({ intentId, ownAgentId, ownAgentAuthorityDid, intent }) {
      ...
      if (intent.kind === "conversation.draft.decide") {
        ...
        if (intent.decision === "confirm" && pairCommunicationState(ownAgentAuthorityDid, conversation2.peerAgentAuthorityDid || conversation2.peerDid) === "revoked") {
    ...
    installPanelRoutes(ctx, webServer, {
      ...
      async listConversations() {
        ...
        return {
          ...
          conversations: Object.values(state.conversations).sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt))).map((c) => ({
            ...
            deliveries: (c.deliveries ?? []).filter((d) => d.state === "pending").map((d) => ({ deliveryId: d.deliveryId, taskId: d.taskId, receivedAt: d.receivedAt })),
      ...
      async conversationMessages(conversationId, cursor, limit) {
        ...
        const snapshot2 = await sessionSnapshot(conversation, cursor, Math.min(Math.max(Number(limit) || 50, 1), 200));
        return {
          ok: true,
          conversationId,
      ...
      async revokePair(localAgentDid, peerAgentDid) {
        ...
        await persistPermissions();
        const paused = Object.values(state.conversations).filter((conversation) => belongsToPair(conversation, localAgentDid, peerAgentDid));
      ...
      async state() {
        ...
        return {
          ...
          conversationWorkspace: {
             ...
             */
            elsewhere: Boolean(
              conversationWorkspace.confirmed && conversationWorkspace.path && conversationWorkspace.path !== workspace
            )
          },
