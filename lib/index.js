...
import { chmodSync, mkdirSync as mkdirSync2, statSync, writeFileSync as writeFileSync2 } from "node:fs";
...
// src/edge/sync.ts
var FREE_TEXT_KEYS = ["title", "reason"];
...
var index_default = {
  ...
  apply(ctx, config = {}) {
    ...
    async function iflowId(args, timeoutSec = 15) {
      ...
      if (!bin) {
        throw new Error(
          `iflow-id binary not found (expected ${join(pluginRoot, "rust", "target", "release")})` + (iflowIdFailure ? `: ${iflowIdFailure}` : "") + ". Run iflow_fetch_identity to retry now and see why."
        );
      }
      const handle = ctx.subprocess.spawn({
    ...
    void (async () => {
      try {
        ...
        edgeHandle = await installIFlowEdge(ctx, {
          ...
          community: config.community && config.community.url && config.community.token ? {
            ...
            intervalMs: Number(config.community.intervalMs) || 6e4
          } : void 0
        });
